...
class CreditoKWESTEKREI(Variables):
    def CreditoKWESTE_KREI(self):
        ...
        df2.to_excel(os.path.join(Variables().ruta_procesados,f'CREDITO_KWESTE_KREI_SDR_{Variables().fechaPath}.xlsx'), index=False)
    def CreditoKWSUR_KREI(self):
        ...
        # exportamos el documento
        df2.to_excel(os.path.join(Variables().ruta_procesados,f'CREDITO_KWSUR_KREI_SDR_{Variables().fechaPath}.xlsx'), index=False)